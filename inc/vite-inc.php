				...
				if ( isset( $manifest_key[0] ) ) {
					...
					if ( isset( $manifest[ $manifest_key[0] ]['css'] ) && is_array( $manifest[ $manifest_key[0] ]['css'] ) ) {
						foreach ( $manifest[ $manifest_key[0] ]['css'] as $css_file ) {
							wp_enqueue_style( 'main-style', DIST_URI . '/' . $css_file, array(), NUTRIFLOW_VERSION );
						}
					}
					...
					if ( isset( $manifest[ $manifest_key[0] ]['file'] ) ) {
						...
						if ( ! empty( $js_file ) ) {
							wp_enqueue_script( 'main-script', DIST_URI . '/' . $js_file, array(), NUTRIFLOW_VERSION, true );
						}
					}
				}
			}
